<div class="w-[560px] max-w-full bg-[#111D30] border border-[#00C853]/15 rounded-3xl overflow-hidden shadow-[0_32px_80px_rgba(0,0,0,0.5),0_0_0_1px_rgba(0,200,83,0.08)]">

    <!-- Header -->
    <div class="relative px-8 pt-7 pb-6 border-b border-[#00C853]/15 bg-[#00C853]/[0.03] flex items-center gap-4">
        ...
        <button class="absolute top-5 right-5 w-9 h-9 bg-[#182236] border border-[#00C853]/15 rounded-xl flex items-center justify-center text-[#8AAABB] hover:text-[#E8F4FF] transition-colors cursor-pointer">
    ...
    <div class="px-8 py-6 flex flex-col gap-6">
        ...
        <!-- Avatar preview -->
        <div class="flex items-center gap-4 bg-[#182236] border border-[#00C853]/15 rounded-2xl px-5 py-4">
        ...
        <div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="flex flex-col gap-1.5">
                    ...
                    <input id="inNombre" class="form-input bg-[#182236] border border-[#00C853]/15 rounded-xl px-3.5 py-[11px] text-[#E8F4FF] text-[13.5px] w-full transition-all" type="text" placeholder="Ej: Luis Miguel" oninput="updatePreview()" />
                </div>
                <div class="flex flex-col gap-1.5">
                    <label class="text-[11px] uppercase tracking-[1px] text-[#8AAABB] font-medium">Apellido <span class="text-[#40C4FF]">*</span></label>
                    <input id="inApellido" class="form-input bg-[#182236] border border-[#00C853]/15 rounded-xl px-3.5 py-[11px] text-[#E8F4FF] text-[13.5px] w-full transition-all" type="text" placeholder="Ej: Muñoz" oninput="updatePreview()" />
                </div>
                <div class="col-span-2 flex flex-col gap-1.5">
                    ...
                    <input class="form-input bg-[#182236] border border-[#00C853]/15 rounded-xl px-3.5 py-[11px] text-[#E8F4FF] text-[13.5px] w-full transition-all" type="email" placeholder="usuario@correo.com" />
                </div>
                <div class="flex flex-col gap-1.5">
                    <label class="text-[11px] uppercase tracking-[1px] text-[#8AAABB] font-medium">Documento</label>
                    <input class="form-input bg-[#182236] border border-[#00C853]/15 rounded-xl px-3.5 py-[11px] text-[#E8F4FF] text-[13.5px] w-full transition-all" type="text" placeholder="N° identificación" />
                </div>
                <div class="flex flex-col gap-1.5">
                    <label class="text-[11px] uppercase tracking-[1px] text-[#8AAABB] font-medium">Estado</label>
                    <select class="form-input bg-[#182236] border border-[#00C853]/15 rounded-xl px-3.5 py-[11px] text-[#E8F4FF] text-[13.5px] w-full transition-all">
                        <option style="background:#111D30">Activo</option>
                        <option style="background:#111D30">Inactivo</option>
                    </select>
                </div>
            </div>
        </div>
    ...
    <!-- Footer -->
    <div class="px-8 py-5 border-t border-[#00C853]/15 flex justify-end gap-2.5">
        <button class="btn-ghost flex items-center gap-2 px-6 py-[11px] rounded-xl text-[13.5px] font-medium text-[#8AAABB] bg-[#182236] border border-[#00C853]/15 cursor-pointer transition-all">
            Cancelar
        </button>
        <button class="btn-primary flex items-center gap-2 px-6 py-[11px] rounded-xl text-[13.5px] font-medium bg-[#40C4FF] text-[#0A1628] cursor-pointer transition-all">
...
<script>
    function updatePreview() {
        const n = document.getElementById('inNombre').value || 'Nuevo';
        const a = document.getElementById('inApellido').value || 'Usuario';
        document.getElementById('avName').textContent = n + ' ' + a;
        document.getElementById('avCircle').textContent = ((n[0] || 'N') + (a[0] || 'U')).toUpperCase();
    }

    function selectRole(name, el, color) {
        const colorMap = {
            green: {
        ...
        document.querySelectorAll('.role-card').forEach(c => {
            c.style.background = '#182236';
            c.style.borderColor = 'rgba(0,200,83,0.15)';
        });
        el.style.background = colorMap[color].bg;
        el.style.borderColor = colorMap[color].border;
        document.getElementById('avRole').textContent = name;
    }

    function toggleProj(el) {
        const isSelected = el.classList.toggle('selected');
        const dot = el.querySelector('.proj-dot');
        const check = el.querySelector('.proj-check');
        if (isSelected) {
            ...
            check.style.background = '#00C853';
            check.style.borderColor = '#00C853';
            check.innerHTML = '<svg width="10" height="10" fill="none" stroke="white" stroke-width="3" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>';
        } else {
            el.style.background = '#182236';
            el.style.borderColor = 'rgba(0,200,83,0.15)';
            if (dot) {
                dot.style.background = '#8AAABB';
            }
            check.style.background = 'transparent';
            check.style.borderColor = 'rgba(138,170,187,0.3)';
            check.innerHTML = '';
        }
    }
